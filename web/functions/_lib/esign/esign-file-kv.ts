...
} from '../construction/construction-limits';
import { base64ToUint8, uint8ToBase64 } from '../construction/construction-binary';

export const ESIGN_MAX_BYTES = CONSTRUCTION_MAX_BYTES;
export const ESIGN_MAX_MB = 15;
export const KV_ESIGN_SINGLE_MAX_BYTES = KV_CONSTRUCTION_SINGLE_MAX_BYTES;

const PREFIX = 'esign:file:';

export function isKvEsignPath(storagePath: string | null | undefined): boolean {
  return Boolean(storagePath?.startsWith('kv:esign:'));
}

export function kvEsignPath(docId: string): string {
  return `kv:esign:${docId}`;
}

export function kvEsignChunkedPath(docId: string): string {
  return `kv:esign:chunked:${docId}`;
}
...
export async function storeEsignFileInKv(
  ...
  if (bytes.length > KV_ESIGN_SINGLE_MAX_BYTES) {
    throw new Error(`File exceeds ${KV_ESIGN_SINGLE_MAX_BYTES / (1024 * 1024)} MB single KV object limit`);
  }
  await env.SETTINGS.put(kvKey(docId), JSON.stringify({ contentType, data: uint8ToBase64(bytes) }));
  return { path: kvEsignPath(docId), contentType };
}

export async function storeEsignFileInKvChunked(
  env: SettingsEnv,
  docId: string,
  bytes: Uint8Array,
  contentType: string,
  ...
  if (bytes.length > ESIGN_MAX_BYTES) {
    throw new Error(`File exceeds ${ESIGN_MAX_BYTES / (1024 * 1024)} MB limit`);
  }
  ...
  return { path: kvEsignChunkedPath(docId), contentType };
}

export async function loadEsignFileFromKv(
  env: SettingsEnv,
  docId: string,
  ...
  if (metaRaw) {
    const meta = JSON.parse(metaRaw) as { contentType: string; parts: number; totalBytes: number };
...
export async function deleteEsignFileFromKv(env: SettingsEnv, docId: string): Promise<void> {
...
export function esignIdFromKvPath(path: string): string {
  return path.replace(/^kv:esign:(?:chunked:)?/, '');
}
