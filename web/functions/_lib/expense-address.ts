...
/** Known non-portfolio service locations on combined bills — never import these. */
const EXCLUDED_SERVICE_RE = [
...
/** Which portfolio property this charge belongs to, from note/vendor text. */
...
export function isPortfolioExpense(fields: {
  ...
  if (
    fields.propertyId &&
    fields.propertyId !== 'ranch' &&
    fields.propertyId !== 'lindon'
  ) {
    return false;
  }
...
/** Drop non-portfolio charges; align propertyId with the service address in the note. */
...
/** @deprecated use mentionsExcludedServiceAddress */
export function matchesExcludedServiceAddress(fields: {
  note?: string;
  vendor?: string;
}): boolean {
  return mentionsExcludedServiceAddress(fields);
}
