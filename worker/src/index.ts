...
let extraCleaningFees: Record<string, number> = { 'ranch-2026-02': 572.92 };
...
function calculateMetrics(propId: PropertyId, currentMonth: string) {
  let revenue = 0;
  let baseCleaning = 0;
  let occupied = 0;
  let stayCount = 0;
...
export default {
  async fetch(request: Request, env: { CORS_ORIGINS?: string }): Promise<Response> {
    ...
    if (url.pathname === '/api/portfolio/metrics' && request.method === 'GET') {
    ...
    if (url.pathname === '/api/portfolio/extra-cleaning' && request.method === 'PUT') {
      ...
      return json(extraCleaningFees, 200, cors);
    }
